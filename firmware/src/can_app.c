...
inline void can_app_send_steeringbat_measurements(void) {
  ...
  msg.data[CAN_MSG_MDE22_STEERINGBAT_MEASUREMENTS_BATVOLTAGE_L_BYTE] =
      LOW(measurements.batvoltage_avg);
  msg.data[CAN_MSG_MDE22_STEERINGBAT_MEASUREMENTS_BATVOLTAGE_H_BYTE] =
      HIGH(measurements.batvoltage_avg);
  msg.data[CAN_MSG_MDE22_STEERINGBAT_MEASUREMENTS_POSITION_L_BYTE] =
      LOW(measurements.position_avg);
  msg.data[CAN_MSG_MDE22_STEERINGBAT_MEASUREMENTS_POSITION_H_BYTE] =
      HIGH(measurements.position_avg);
  msg.data[CAN_MSG_MDE22_STEERINGBAT_MEASUREMENTS_BATCURRENT_L_BYTE] =
      LOW(measurements.batcurrent_avg);
  msg.data[CAN_MSG_MDE22_STEERINGBAT_MEASUREMENTS_BATCURRENT_H_BYTE] =
      HIGH(measurements.batcurrent_avg);
...
inline void can_app_extractor_mic19_mde(can_t *msg) {
  ...
  if (msg->data[CAN_MSG_MIC19_MDE_SIGNATURE_BYTE] == CAN_SIGNATURE_MIC19) {
    HIGH_LOW(str_whl_tmp, msg->data[CAN_MSG_MIC19_MDE_POSITION_H_BYTE],
             msg->data[CAN_MSG_MIC19_MDE_POSITION_L_BYTE]);
    can_app_checks_without_mic19_msg = 0;
...
/**
 * @brief extracts MIC19 pump message being used to define hbridge Kp
 * @param *msg pointer to the message to be extracted
 */
inline void can_app_extractor_mic19_pumps(can_t *msg) {
  if (msg->data[CAN_MSG_MIC19_PUMPS_SIGNATURE_BYTE] == CAN_SIGNATURE_MIC19) {
    if (msg->data[CAN_MSG_MIC19_PUMPS_PUMPS_BYTE] && 0x01) // bit related to pump1
      sense_select = 1;
    else
      sense_select = 0;
  } else {
...
#ifdef USART_ON
      VERBOSE_MSG_CAN_APP(usart_send_string("got MIC pumps msg: "));
#endif
      VERBOSE_MSG_CAN_APP(can_app_print_msg(msg));
...
inline void check_can(void) {
  ...
  // CAN_APP_CHECKS_WITHOUT_MIC19_MSG cycles, than it go to a specific error
  // state.
