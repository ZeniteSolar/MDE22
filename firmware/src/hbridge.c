...
void hbridge_init(void) {
  TCCR0A =
      (1 << COM0A1) | (1 << COM0A0)   // Set on compare match when up-counting,
                                      // clear ... on down-counting
      | (1 << COM0B1) | (0 << COM0B0) // Clear on compare match when
                                      // up-counting, set ... on down-counting
      | (0 << WGM01) | (1 << WGM00);

  // fOCnxPCPWM = (fclk_IO)/(N x 510), N being the prescaler
  // Prescaler 010 = 8 -> 3921.5 Hz               // Used IC operates up to
  // 25kHz, this timer presents minimum prescaler of either 1 or 8
...
static float e1 = 0;
static float u = 0;
static uint16_t hbridge_send_clk;

float PI(float r, float y) {
  // PI CONFIGURATIONS:
  if (sense_select == 1)
    Kp = 2.5; // analog series proportional gain
  else if (sense_select == 0)
    Kp = 1.4; // analog series proportional gain // TODO fine tune
  else {
#ifdef VERBOSE_MSG_HBRIDGE
    VERBOSE_MSG_HBRIDGE(usart_send_string("CORRUPT sense select value "));
#endif
  }
  // Compute error:
  e1 = r - y;
...
#ifdef VERBOSE_MSG_HBRIDGE
  if (++hbridge_send_clk >= 50) {
    hbridge_send_clk = 0;
    VERBOSE_MSG_HBRIDGE(usart_send_float(position_setpoint));
    VERBOSE_MSG_HBRIDGE(usart_send_string(","));
    VERBOSE_MSG_HBRIDGE(usart_send_float(angle));
    VERBOSE_MSG_HBRIDGE(usart_send_string(","));
    VERBOSE_MSG_HBRIDGE(usart_send_float(dt * 99));
    VERBOSE_MSG_HBRIDGE(usart_send_char(','));
    VERBOSE_MSG_HBRIDGE(usart_send_float(batvoltage));
    VERBOSE_MSG_HBRIDGE(usart_send_char(','));
    VERBOSE_MSG_HBRIDGE(usart_send_float(batcurrent));
    VERBOSE_MSG_HBRIDGE(usart_send_char('\n'));
  }
#endif
...
void hbridge_control(float angle) {
  ...
  if (position_setpoint > STR_MAX_ANGLE)
    position_setpoint = STR_MAX_ANGLE;
  if (position_setpoint < STR_MIN_ANGLE)
    position_setpoint = STR_MIN_ANGLE;
